...
import { T_Game_Fowl_Body_Color, T_Game_Fowl_Class, T_Game_Fowl_Gender } from "@repo/contract";
import Link from "next/link";
import { LucideBookText, LucideGitPullRequestCreateArrow, LucideSquareGantt } from "lucide-react";
...
const View = () => {
  ...
  const gameFowlGender = data?.items[0];
  ...
  const classTitle = (item: T_Game_Fowl_Class) => item.classTitle || "Unknown";
  ...
  const openModal = (gameFowlBodyColor: T_Game_Fowl_Body_Color) => {
    setBodyColorToDelete(gameFowlGender);
    setIsModalOpen(true);
  };
  ...
  gameFowlGender?.gameFowlGender?.filter((gameFowlGender: T_Game_Fowl_Gender) => {
      const titleMatch = gameFowlGender.genderTitle
        ?.toLowerCase()
  ...
  const renderRow = (row: T_Game_Fowl_Gender) => (
    <TableRow key={row._id}>
      <TableCell className="w-3/4">
        <Typography>{row.genderTitle}</Typography>
      </TableCell>
      <TableCell className="w-1/4 flex gap-4">
        <Button variant="outline">Hide</Button>
        <Link href={`/admin/settings/gf-gender/${row._id}/edit`}>
          <Button>Edit</Button>
        </Link>
      </TableCell>
    </TableRow>
            ...
            <div className="border-2 shadow rounded-md p-5 flex flex-col min-h-screen">
              <div className="flex gap-2 items-center">
                ...
                <Typography variant="h3" fontWeight="semiBold">
                  View lists of game fowl gender
                </Typography>
              </div>
