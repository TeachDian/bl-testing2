...
import useAddGameFowlGender from "../hooks/useAddGameFowlGender";
...
const AddGender = () => {
  const { mutateAsync: addGender } = useAddGameFowlGender();
  const { register, handleSubmit } = useForm<T_Game_Fowl_Gender>();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const onSubmit = async (formData: T_Game_Fowl_Gender) => {
    try {
      const modifiedDataForm = {
        ...formData,
        genderTitle: capitalizeFirstLetter(formData.genderTitle),
      };
      const response = await addGender(modifiedDataForm);
