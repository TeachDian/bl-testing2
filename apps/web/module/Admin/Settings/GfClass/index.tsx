...
import useAddGameFowlClass from "../hooks/useAddGameFowlClass";
...
const AddFederation = () => {
  const { mutateAsync: addClass } = useAddGameFowlClass();
  const { register, handleSubmit } = useForm<T_Game_Fowl_Class>();
  ...
  const onSubmit = async (formData: T_Game_Fowl_Class) => {
    try {
      ...
      const response = await addClass(modifiedDataForm);
              ...
              >
                <div>
                  <div className="sm:flex items-center mb-4 gap-2">
                    ...
                    <Input
                      placeholder="Enter game fowl class"
                      required
                      {...register("classTitle", {
