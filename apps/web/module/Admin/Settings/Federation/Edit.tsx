...
import useUpdateGameFowlFederation from "../hooks/useUpdateGameFowlFederation";
import useGetGameFowlFederationById from "../hooks/useGetGameFowlFederationById";
...
const EditFederation = () => {
  const params = useParams<{ federationId: string }>();
  const federationId = params.federationId;
  const { mutateAsync: updateFederation } =
    useUpdateGameFowlFederation(federationId);
  const { setValue, register, handleSubmit } =
    useForm<T_Game_Fowl_Federation>();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const { data, isPending } = useGetGameFowlFederationById(federationId);
  ...
  useEffect(() => {
    if (!isPending && data?.item) {
      ...
      setValue("federationTitle", data.item.federationTitle || " ");
    }
