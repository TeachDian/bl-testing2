...
import useAddGameFowlSkillsCategory from "../hooks/useAddGameFowlSkillsCategory";
...
const AddSkillsCategory = () => {
  const { mutateAsync: addSkillsCategory } = useAddGameFowlSkillsCategory();
  const { register, handleSubmit } = useForm<T_Game_Fowl_Skills_Category>();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const onSubmit = async (formData: T_Game_Fowl_Skills_Category) => {
    try {
      const modifiedFormData = {
        ...formData,
        skillsCategory: capitalizeFirstLetter(formData.skillsCategory),
      };
      const response = await addSkillsCategory(modifiedFormData);
        ...
        <div className="border-2 shadow rounded-md sm:h-screen h-auto p-5 flex flex-col">
          <div className="flex gap-2 items-center">
            ...
            <Typography variant="h3" fontWeight="semiBold">
              Set up game fowl skills category
            </Typography>
          </div>
