...
import { LucideBookOpen, LucideHourglass, LucidePenLine } from "lucide-react";
import { useState } from "react";
import useUpdateGameFowlStatus from "../hooks/useUpdateGameFowlStatus";
import toast from "react-hot-toast";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import useAddGameFowlStatus from "../hooks/useAddGameFowlStatus";
  ...
  const onSubmit = async (formData: T_Game_Fowl_Status) => {
    try {
      const modifiedFormData = {
        ...formData,
        gameFowlStatus: capitalizeFirstLetter(formData.gameFowlStatus),
        description: capitalizeFirstLetter(formData.description),
      };

      const { mutateAsync: updateGameFowlStatus } = useAddGameFowlStatus();
      const response = await updateGameFowlStatus(modifiedFormData);
              ...
              >
                ...
                <div className="flex gap-4">
                  ...
                  <Button
                    ...
                    variant="destructive"
                    size="lg"
                    // Add click handler for "Back" button if needed
                  >
                    Back
                  </Button>
                </div>
